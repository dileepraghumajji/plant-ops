...
10. **Who is bound to the restricted on-prem platform role (§6.4)?** The role carries `iam.platform.*` keys, so it lives in the platform tenant and binds at the platform scope root — while the client's IT are users of the *client* tenant, and a `role_binding` cannot cross that line. Delivering §6.4 as written therefore means creating **a second human identity at install**, in the platform tenant, with its own password in `.env` and its own place in the install guide. The alternatives are to ship the role as a definition nobody is bound to and walk an operator through binding themselves in the runbook, or to abandon the separate role and grant the reads to the client admin — which would put `iam.platform.*` on a client-tenant role and contradict the tier split. **Session 45 is blocked on this**, deliberately: it is a decision about what the product is, not an implementation detail.
