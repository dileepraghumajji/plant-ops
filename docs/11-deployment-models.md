...
Session 41 did the other half: `deploy/proxy/` serves the console at `/` and the API at `/api` with the prefix stripped, on whatever hostname arrives (`server_name _`). The console image is built with no `NEXT_PUBLIC_*` at all and `.dockerignore` excludes every `.env*` so a developer's local file cannot be swept into the bundle. A same-origin deployment needs no `CORS_ALLOWED_ORIGINS` entry, which is why the stack sets it empty.

**3. ~~The database configuration assumes a Supabase pooler.~~ Closed.** Session 46 replaced the assumption with two statements of fact.

`DATABASE_POOLED` says whether a transaction-mode pooler is in front of `DATABASE_URL`. It defaults to `true`, so a managed deployment upgrades into the new schema without editing anything and behaves exactly as it did; the bundled stack and every on-premise install set `false`. **Both URLs naming the same host and port is now a supported, documented configuration** — the schema only objects to that shape in combination with `DATABASE_POOLED=true`, which claims a pooler that is not there, and the error names both ways of fixing it. What the two URLs must still never share is the role (Doc 07 §5.1); that split is orthogonal to all of this and remains enforced at boot by `startup-checks.ts`.

On prepared statements the honest answer turned out to be that there is no flag to move: TypeORM's Postgres driver never supplies a statement name, and node-postgres prepares only *named* queries, so nothing is ever cached into a server connection a pooler could hand to another client. The pooled requirement is therefore met structurally rather than by configuration, in both modes. `usesPreparedStatements()` in `libs/db` reports the permission — for the boot log, which is how an install we cannot reach says which topology it believes it is in, and for any future code that would name a statement and must ask first.

`DATABASE_SSL` grew from a boolean into a mode: `disable`, `verify-ca`, `verify-full` (`true`/`false` still parse, as `verify-full`/`disable`). `verify-ca` is the one this gap was really about — it verifies the chain against a `DATABASE_CA_CERT` it *requires* and relaxes only the hostname check, which is what a client terminating TLS at their own Postgres needs when their certificate names the server and the connection string names a compose service or a virtual address. libpq's `require`, `prefer` and `allow` are deliberately not offered: all three encrypt without authenticating, and there is still no setting anywhere that does that. The bundled container needs no TLS configuration at all.

**4. ~~There is no single-tenant mode.~~ Closed.** Session 44 added `DEPLOYMENT_MODE` (`saas` | `single_tenant`) and `SINGLE_TENANT_CLIENT_SLUG`. In single-tenant mode the pinned client is resolved **at boot**, through a `security definer` lookup (migration 0018) because there is no RLS context before the first request; the API refuses to start if the slug names nothing, rather than coming up healthy and answering 401 to every login.

The control is not the missing form field. `auth/single-tenant.middleware.ts` supplies `client_slug` from configuration on `POST /auth/login` and **refuses** a request that names a different tenant — so a caller can neither choose a tenant nor believe they did. Nothing beneath that changed: `loginSchema` still requires the field, `AuthService` still resolves a tenant by slug, `app.current_client_id` still comes from verified claims, and the whole existing e2e battery passes against a `saas` instance unmodified, which is the evidence that it did.

Client creation is refused in single-tenant mode as a coherence rule (§6.5): a second `client` row would be unreachable by every request the process serves. `GET /iam/deployment` tells the origin-agnostic console which form to render, and `single-tenant.e2e.ts` runs a second API instance in the other mode to assert all of it.
...
4. ~~**Break-glass as a command, or as a standing permission?**~~ **Decided: the command.** Session 45 ships `tools/break-glass-admin.ts`, run on the host in the operations container, gated on the *current* platform credential verified against its stored hash, and audited as `platform.break_glass` with the operator's stated reason. `iam.platform.client.admin.create` is in no role. The 3am cost is real and was accepted: recovery needs a shell on the server rather than a browser. What bought it is that the capability cannot be exercised quietly — the alternative would have sat in a role for the life of the deployment, usable by whoever held it, recorded as an ordinary `user.created`.
...
The five below came out of building sessions 41–44. They are open in the same sense: each has a defensible default in the code today, and each would be cheap to change *now* and expensive to change after a pilot has shipped.

6. **Does a release get a git tag?** The repository has none, so CI computes `0.0.0-<12-char sha>` and that is what `/health` reports and what every image is tagged with. It is honest — it says "not a release" — and it is not a version number a customer can say out loud. `git describe --tags --dirty` already prefers a real tag the moment one exists, so this is a `git tag` and no code change. Decide before the first bundle leaves the building, because the tag is baked into the artifact's filename, its images and its `.env`.

7. **Is the bundle signed?** §5.3 says "digest-pinned", and `MANIFEST.json` records the digest each image was built from — but nothing signs the manifest, so a client told to trust a tarball has no way to verify it. Session 48 introduces an offline-verifiable licence and will need a signing key regardless; the sensible move is one key and one `--verify` command covering both. The alternative is to accept that the tarball is trusted because of how it was delivered, which is defensible for a hand-carried pilot and not for a download link.

8. **Must the first administrator change their password?** The installer creates them from `.env`, which means the credential exists on disk in plaintext until somebody removes it, and `./bootstrap.sh --verify` needs it to prove login works. The install output says to change it; nothing enforces it. A "must change on first login" flag is the real answer and is a schema and login-path change — worth scheduling deliberately rather than discovering during a security review.

9. **How does an upgrade get platform authority?** Re-applying the release's manifests (§6.3) needs `iam.platform.app.manifest`, and a finished install deliberately holds no platform credential (Doc 07 §8). Today `deploy/README.md` §5 tells the operator to put `PLATFORM_BOOTSTRAP_SECRET` back for the length of the upgrade. That works and is the weakest seam in the install story. The better answer is a **dedicated release identity** — a service account created at bootstrap holding only the manifest permissions, useless for anything else, its secret living in `.env` for good. It is small, and it would also let the upgrade re-apply the catalog automatically instead of asking.

10. ~~**Who is bound to the restricted on-prem platform role (§6.4)?**~~ **Decided: a platform-tenant service account, `onprem-operator`.**

    Building it surfaced two facts that narrowed the choice. First, a platform-tenant *human* could not sign in at all: `applyRlsContext` derives platform authority only from a token whose `cid` is already the platform client, and in `single_tenant` mode `SingleTenantLoginMiddleware` refuses every `client_slug` but the pinned one — so delivering §6.4's console story would have meant reopening the login path Session 44 was deliberately careful with. Second, there is no API that could have made the binding anyway: the platform tier holds no `iam.client.user.create` or `iam.client.binding.create` (migration 0017), so *any* subject bound to this role has to be seeded with SQL.

    A service account is what platform authority already is in this system — `platform-bootstrap` is one — so migration 0019 provisions the identity, binds it at the platform scope root, and gives it a secret it generates and discards. `./bootstrap.sh --onprem-credential` issues a usable one on demand through the ordinary rotate endpoint, printed once, so nothing lives in `.env` by default and decision 8's smell is not repeated.

    **What this does not deliver, stated plainly:** the client's IT get read-only visibility through the *API*, not through the console. §6.4's "ungranted actions degrade to a disabled control" is still true of the console and is verified — every platform screen gates on its individual permission, and the manifest screen renders its 403 in place — but a machine identity does not sign in, so nobody sees those screens. If a client asks for the console specifically, that is the human-identity option above, and it is a login-path change to be specced rather than a flag to flip.
