...
> **Not every deployment has a pooler** (Session 46, Doc 11 §8 gap 3). The two bullets above describe the managed target; an on-premise install runs one Postgres and points both URLs at it. So the pooling decision is `DATABASE_POOLED`, stated in configuration, and never inferred from the endpoints: `true` is the default and preserves the managed behaviour exactly, `false` says there is nothing in front of the application's connection. Both URLs naming the same host and port is a **supported, documented configuration** — the schema only refuses it in combination with `DATABASE_POOLED=true`, which claims a pooler that is not there. What the two URLs must never share is the role (§5.1).
>
> On prepared statements specifically: there is no TypeORM flag to disable, and none is needed. Its Postgres driver calls `client.query(sql, params)` without a statement name, and node-postgres prepares only *named* queries — so no statement is ever cached into a server connection the pooler may hand to another client. The requirement is met structurally, and `usesPreparedStatements()` in `libs/db` reports the permission for any future code that would name one.
>
> If you do put your own pooler in front, its `default_pool_size` must be at least `APP_POOL_SIZE` (10, `libs/db/src/data-source.ts`) per API instance. Measured, with a PgBouncer sized at 5 below an application pool of 10: the concurrent-refresh election in Doc 03 §4 deadlocks — a transaction holding a server connection waits on a row lock held by a transaction that cannot get one — and the requests fail with connection errors that name nothing. Sized above the application pool, the same suite passes in full.
>
> TLS is likewise not a boolean. `DATABASE_SSL` takes `disable`, `verify-ca` or `verify-full` (`true`/`false` still parse, as `verify-full`/`disable`). `verify-ca` verifies the chain against an operator-supplied `DATABASE_CA_CERT` and skips only the hostname check — the case of a client whose own certificate authority issued a certificate for a Postgres reached by a service name or a virtual address that certificate does not carry; it requires the anchor. libpq's `require`, `prefer` and `allow` are deliberately not offered: all three encrypt without authenticating the server, and §5.1 treats the database as the last line of defence.
