...
- **CI does not publish images.** The `images` job builds all four release images, tags them with one computed version, and runs them; it pushes none of them. Railway builds what it deploys, and a second registry copy would mean two artifacts with one version number. The artifact a self-hosted install actually receives is the offline tarball of Session 42, produced from exactly these builds.
- **~~The deployed version is not visible at runtime.~~** `GET /health` now reports `APP_VERSION`, stamped into every image at build time and carried as an OCI label as well, so a stopped container can still be asked. On this pipeline the value is `0.0.0-<sha>` — the repository has no tags yet, and a version that says "not a release" is better than one that claims a number nobody cut. CI fails if the tag and the reported version diverge.
- **Migrations still run from CI on this path, not from the release image.** That is fine here — the runner has a Node toolchain, and `release-migrate` is what enforces the ordering. `plantops/migrate` (Session 41) exists for the path where it is not fine: a client's server, with no toolchain and no checkout. Either way it is the same script against the same direct URL.
