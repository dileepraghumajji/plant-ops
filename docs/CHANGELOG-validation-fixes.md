...
**19. Refresh-reuse detection revoked the session in the database but never told the revocation cache**
*High severity.* `apps/iam-api/src/auth/refresh.service.ts` (one call added) · `refresh.service.spec.ts` (three cases) · Doc 03 §4/§6 (behaviour unchanged — it now happens).
Found in Session 38, by the first suite to exercise reuse detection against a **real** Redis. `iam.auth_rotate_refresh_token` does everything Doc 03 §4 asks on the `reuse` branch: it sets `revoked_at` and writes `auth.refresh.reuse_detected`, atomically, before returning. `RefreshService` then refused the request and did nothing else — and the comment there said so explicitly ("nothing to add"). But the database is not what `AuthGuard` reads: the hot path checks the revoked-`sid` entry in Redis (Doc 03 §6), and falls back to `session_is_revoked()` only when the cache cannot answer. With Redis healthy — i.e. in production — the entry was never written, so **the access token that arrived alongside the stolen refresh token went on authenticating every request for the remainder of its TTL**, up to fifteen minutes. Measured over HTTP against the built bundle: after a two-generations-old refresh token was replayed, `POST /auth/refresh` correctly returned 401 while `GET /iam/whoami` with the paired access token returned 200.

The shape is the one this document keeps recording: nothing failed, no test went red, and the audit trail said the compromise had been handled. Every *other* revocation path — logout, force-revoke, lock, disable, password reset — goes through `SessionService.publishRevocations`, so the gap was in the one branch that reaches the database through a `SECURITY DEFINER` function instead of through the service that owns publishing.

**Decision:** the `reuse` branch calls `publishRevocations([sessionId])` before refusing. Directly, not deferred: the rotation function is a single statement that has already committed by the time it returns, so there is no transaction to hang an `afterCommit` on. Publishing stays best-effort, exactly as everywhere else — the row is authoritative and the guard's database fallback closes a failed publish. `stale` and `invalid` publish nothing, and are tested for it: neither is a compromise, and revoking on `stale` would kill a live session because a cache lost an election.
